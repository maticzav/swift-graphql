    ...
    func register(
        ...
        let downstream = shared
            .filter({ operation in
                ...
                // We only cache queries and mutations and ignore all other types
                // of transactions.
                guard operation.kind == .query || operation.kind == .mutation else {
                    return true
                }
                
                // Filter out cache-first requests that were matched/hit.
                let wasHit = self.resultCache[operation.id] != nil
