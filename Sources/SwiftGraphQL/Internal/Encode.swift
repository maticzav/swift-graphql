...
/**
 This is a cleaned up copy of JSONEncoder from Swift's source renamed to Variable encoder.
 The only difference is in the return type of `encode` function which now - instead of Data - returns
 a NSObject.

 The last part - previously done by `encode` function is  handled by `perform` method inside the client that
 you can find in `HTTP` file.
...
open class VariableEncoder {
    ...
    open func encode<T: Encodable>(_ value: T) throws -> NSObject {
        let encoder = __JSONEncoder(options: options)
...
private class __JSONEncoder: Encoder {
    ...
    let options: VariableEncoder._Options
    ...
    init(options: VariableEncoder._Options, codingPath: [CodingKey] = []) {
        self.options = options
        storage = _JSONEncodingStorage()
    ...
    public func container<Key>(keyedBy _: Key.Type) -> KeyedEncodingContainer<Key> {
        ...
        let container = _JSONKeyedEncodingContainer<Key>(referencing: self, codingPath: codingPath, wrapping: topContainer)
...
private struct _JSONKeyedEncodingContainer<K: CodingKey>: KeyedEncodingContainerProtocol {
    ...
    private func _converted(_ key: CodingKey) -> CodingKey {
        ...
        case .useDefaultKeys:
            return key
        case .convertToSnakeCase:
            let newKeyString = VariableEncoder.KeyEncodingStrategy._convertToSnakeCase(key.stringValue)
    ...
    public mutating func nestedUnkeyedContainer(forKey key: Key) -> UnkeyedEncodingContainer {
        let containerKey = _converted(key).stringValue
        let array: NSMutableArray
        if let existingContainer = container[containerKey] {
            precondition(
    ...
    public mutating func superEncoder() -> Encoder {
        return __JSONReferencingEncoder(referencing: encoder, key: _JSONKey.super, convertedKey: _converted(_JSONKey.super), wrapping: container)
    }

    public mutating func superEncoder(forKey key: Key) -> Encoder {
        return __JSONReferencingEncoder(referencing: encoder, key: key, convertedKey: _converted(key), wrapping: container)
    }
}
...
private struct _JSONUnkeyedEncodingContainer: UnkeyedEncodingContainer {
    ...
    public mutating func superEncoder() -> Encoder {
        return __JSONReferencingEncoder(referencing: encoder, at: container.count, wrapping: container)
    }
}
    ...
    func box(_ float: Float) throws -> NSObject {
        guard !float.isInfinite, !float.isNaN else {
            guard case let .convertToString(positiveInfinity: posInfString,
                                            ...
                                            nan: nanString) = options.nonConformingFloatEncodingStrategy
            else {
    ...
    func box(_ dict: [String: Encodable]) throws -> NSObject? {
        ...
        do {
            for (key, value) in dict {
                codingPath.append(_JSONKey(stringValue: key, intValue: nil))
                defer { self.codingPath.removeLast() }
...
private class __JSONReferencingEncoder: __JSONEncoder {
    ...
    init(referencing encoder: __JSONEncoder, at index: Int, wrapping array: NSMutableArray) {
        ...
        codingPath.append(_JSONKey(index: index))
    }
