...
SwiftGraphQLClient comes with a special exchange dedicated to authenticating your requests - `AuthExchange`. You provide a function that
returns a token and it applies that token to a given header to make an authenticated request.
